...
4. **PlayerSeeder + PlayerContext** — Queue seeding is split into two layers:
   ...
   - **30-second preview gating** — tracks that are **not** part of the admin-curated playlist play only a 30-second preview, then auto-advance. The store holds `curatedTrackIds: Set<string>` + `hasCuratedPlaylist: boolean`, populated via `registerPlaylistIds(ids, curated)` at every playlist source (`PlayerSeeder` server props, `ensureQueueSeeded` fallback, low-queue refetch). `isPreviewOnly(state, trackId)` (exported from `playerStore.ts`) returns true only when a curated playlist exists AND the track isn't in it. `PlayerContext`'s `timeupdate` handler enforces the cap: when `currentTime >= PREVIEW_LIMIT_SECONDS` (30) on a preview-only track it advances like a track-end (`previewAdvancedRef` guards against double-firing; reset per track). When there is **no** admin-curated playlist (`isAdminCurated=false`), gating is disabled and every track plays in full.
...
19. **Portal Room landing (`/`)** — Immersive 3D landing experience. `page.tsx` fetches published albums server-side, validates them through `toPortalAlbums()` (`src/lib/portalData.ts` — hex-color validation, cover via `getAlbumCover(cover, 1024)`, drops albums without covers), and renders `PortalRoomClient` (`src/components/portal/`). One album portal fills the viewport at a time; swipe (mobile, `@use-gesture/react`), on-screen arrows + ArrowLeft/Right keys + horizontal trackpad scroll (desktop) move between portals. Tap/click the portal (or the Enter Portal CTA) triggers a fly-through (camera push-in + accent-color energy fill via framer-motion) then routes to the existing `/album/[slug]` page. 3D layer is React Three Fiber + drei (pinned: three 0.185.1, @react-three/fiber 9.6.1, @react-three/drei 10.7.7), dynamic-imported `ssr:false` so only `/` pays the cost; `PerformanceMonitor` degrades quality (dpr 1, fewer particles) on slow devices; dpr capped at 2; only active portal ±1 are mounted so textures lazy-load. Fail-closed: no WebGL or any scene runtime error → polished 2D fallback (`PortalFallback2D`, same navigation, CSS glow); `prefers-reduced-motion` disables particles/shimmer/fly-through (crossfade instead). A11y: DOM-layer controls (canvas is `aria-hidden`), `aria-live` announces "Title, portal N of M", 44px+ touch targets, visible focus rings. Active index persists in `?portal=` + sessionStorage so back-navigation restores the focused portal. Classic home (`HomeClient`) remains as the fallback when no album qualifies for a portal. PayPal, Supabase auth, and purchase APIs are untouched; no CSP changes were needed.
